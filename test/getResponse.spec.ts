...
describe('Transformer', () => {
    it.each(cases)('should return response %s', (i) =>
        expect({ ...getResponse(i.request, localData) }).toEqual(i.response),
    );

    it.each(casesWithFilter)('should return response %s', (i) => {
        const result = getResponse(i.request, localData);
    ...
    it.each(casesWithMissingValues)('should return response with missing data and %s', (i) =>
        expect({ ...getResponse(i.request, payloadUndefined) }).toEqual(i.response),
    );

    it('should handle free text search with null values', () => {
        expect({ ...getResponse(microsoftSearchRequest, payloadNull) }).toEqual(searchTextMicrosoftNullValuesResponse);
    });
});
