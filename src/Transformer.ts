...
import { AggregateFunctions, ColumnSortDirection, CompareOperators } from './Models';
...
export default class Transformer {
    ...
    private static applyFiltering(request: GridRequest, subset: any[]) {
        request.Columns.filter((column: ColumnModel) => column.hasFilter).forEach((column: ColumnModel) => {
            const isDate =
                column.DataType === 'datetime' || column.DataType === 'date' || column.DataType === 'datetimeutc';

            const partialfiltering = (data: any[], action: (f: string) => boolean) =>
                data.filter((row: any) =>
            ...
            switch (column.Filter.Operator) {
                ...
                case CompareOperators.BETWEEN:
                    if (isDate) {
                        ...
                        subset = subset.filter(
                            row =>
                                row[column.Name] >= column.Filter.Text && row[column.Name] <= column.Filter.Argument[0],
                        );
                    }
                    break;
                default:
    ...
    private static applySorting(request: GridRequest, subset: any[]) {
        ...
        let sorts: any[] = [{ Name: request.Columns[0].Name, Asc: true }];
